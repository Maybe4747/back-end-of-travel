...
const server = http.createServer((req, res) => {
  ...
  if (pathname === "/api/notes") {
    ...
    const foundUser = user.find((u) =>
      u["user_info"].nickname.toLowerCase().includes(keyword)
    );
    let filteredNotes = [];
    ...
    if (foundUser) {
      ...
      filteredNotes = notes.filter(
        (note) => note.user_id === userId && !note.isDeleted
      );
    }
    ...
    const uniqueNotes = [
      ...new Map(
        [...filteredNotes, ...titleFilteredNotes].map((note) => [note.id, note])
      ).values(),
    ];
    ...
    if (req.method === "POST") {
      ...
      req.on("end", () => {
        try {
          ...
          foundUser.user_info.follow = foundUser.user_info.follow.filter(
            (id) => id !== followId
          );
          followUser.user_info.fans = followUser.user_info.fans.filter(
            (id) => id !== userId
          );
